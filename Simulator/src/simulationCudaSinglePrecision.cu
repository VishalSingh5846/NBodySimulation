...
__device__ void magnitude(Vec3* vec, float* mag){
...
__device__ void multiply(Vec3* vec, float k){
    vec->x *= k;
    vec->y *= k;
    vec->z *= k;
}
...
__device__ float TimeSpecToSeconds(struct timespec* ts){
...
__device__ float distance(Vec3 &p1, Vec3 &p2){
